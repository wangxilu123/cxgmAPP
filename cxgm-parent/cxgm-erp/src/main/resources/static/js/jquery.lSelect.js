 ...
 *
 *  TODO:SUN.AO改造2016.07.12 amazeUI样式
 **/
...
$.extend($.fn, {
	lSelect: function(options){
		...
		var settings = {
			...
			cssStyle: {"margin-bottom": "0.5rem"}// 下拉框左右css样式
		};
		...
		return this.each(function(){
			...
			function addSelect(element, selected) {
				...
				if(element.is("select")) {
					element.next().nextAll().remove();
					if(element.val() == "") {
						return;
					}
					element.parent().append('<select class="' + settings.cssClass + ' ' + selectGroupClass + '"></select>');
					$nextSelect = element.nextAll("." + selectGroupClass).first();
				...
				if (isFill) {
					if($nextSelect.is("select")){
						$nextSelect.next().css(settings.cssStyle);
					}
